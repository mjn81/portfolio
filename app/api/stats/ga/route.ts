...
async function fetchGASummary(
	...
	try {
		...
		if (response.rows && response.rows.length > 0) {
			...
			const avgSessionDurationSec = parseFloat(
		...
		throw new Error(
			`GA API Error (Summary): ${error.message || 'Unknown error'}`
		);
	}
}

// Fetch Daily Stats (Views, Visitors per day)
async function fetchGADaily(
	client: BetaAnalyticsDataClient,
	dateRange: { startDate: string; endDate: string }
) {
	try {
		...
		throw new Error(
			`GA API Error (Daily): ${error.message || 'Unknown error'}`
		);
	}
}

// Fetch Top Pages
async function fetchGATopPages(
	client: BetaAnalyticsDataClient,
	dateRange: { startDate: string; endDate: string },
	limit: number = 10
) {
	try {
		...
		throw new Error(
			`GA API Error (Top Pages): ${error.message || 'Unknown error'}`
		);
	}
}

// Fetch Top Referrers
async function fetchGATopReferrers(
	client: BetaAnalyticsDataClient,
	dateRange: { startDate: string; endDate: string },
	limit: number = 10
) {
	try {
		...
		throw new Error(
			`GA API Error (Referrers): ${error.message || 'Unknown error'}`
		);
	}
}

// Fetch Device Breakdown
async function fetchGADevices(
	client: BetaAnalyticsDataClient,
	dateRange: { startDate: string; endDate: string }
) {
	try {
		...
		throw new Error(
			`GA API Error (Devices): ${error.message || 'Unknown error'}`
		);
	}
}
...
export async function GET(request: NextRequest) {
	...
	if (!analyticsDataClient || credentialsError) {
		...
		const errorMessage =
			credentialsError || 'Google Analytics client not initialized.';
	...
	if (cachedData) {
		...
		return NextResponse.json(
			{ data: cachedData, placeholder: false, message: null, cached: true },
			{ status: 200 }
		);
	}
	...
	try {
		...
		if (metric === 'summary') {
			...
			return NextResponse.json(
				{ error: 'Invalid metric for GA data' },
				{ status: 400 }
			);
		}
		...
		return NextResponse.json(
			{ data, placeholder: false, message: null, cached: false },
			{ status: 200 }
		);
	} catch (error: any) {
